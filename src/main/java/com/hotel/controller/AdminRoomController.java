    ...
    public ResponseEntity<?> addRoomType(@RequestBody RoomType roomType) {
        try {
            ...
            if (roomType.getName() == null || roomType.getName().trim().isEmpty()) {
                ...
                errorResponse.put("success", false);
                errorResponse.put("code", 400);
                errorResponse.put("message", "房型名称不能为空");
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorResponse);
            }
            ...
            if (roomType.getBasePrice() == null || roomType.getBasePrice().doubleValue() <= 0) {
                ...
                errorResponse.put("success", false);
                errorResponse.put("code", 400);
                errorResponse.put("message", "基础价格必须大于0");
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorResponse);
            }
            ...
            if (roomType.getCapacity() == null || roomType.getCapacity() <= 0) {
                ...
                errorResponse.put("success", false);
                errorResponse.put("code", 400);
                errorResponse.put("message", "可住人数必须大于0");
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorResponse);
            }
            ...
            if (roomTypeRepository.existsByName(roomType.getName())) {
                ...
                errorResponse.put("success", false);
                errorResponse.put("code", 400);
                errorResponse.put("message", "房型名称已存在");
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(errorResponse);
            }
            ...
            Map<String, Object> data = new HashMap<>();
            data.put("id", savedRoomType.getId());
            data.put("name", savedRoomType.getName());
            data.put("basePrice", savedRoomType.getBasePrice());
            data.put("capacity", savedRoomType.getCapacity());
            data.put("amenities", savedRoomType.getAmenities());
            data.put("description", savedRoomType.getDescription());
            data.put("createTime", savedRoomType.getCreateTime());
